...
class TrainerParams:
    def __init__(
        ...
        print(f'{self.device}, {self.local_rank}, {self.save_every}, {self.batch_size}, {self.gradient_accumulation_iter}')
...
def TrainingProcessMain(args):
    ...
    if torch.accelerator.is_available():
        ...
        print(f"Running on rank {rank} on device {my_device}")
    else:
        my_device = torch.device("cpu")
        print(f"Running on device {my_device}")

    #instantiate a local object just to keep tracks of all the params
    local_train_pparam = TrainerParams(Rank=rank, Device=my_device, BatchSize=args.batch_size, GradientAccumulationIteration=args.gradient_accumulation_iter, SaveEvery=args.save_every)
    
    #init process group using default backend, this is required for DDP and FSDP
    fsdp2_setup(device=local_train_pparam.device, rank=local_train_pparam.local_rank)

    
    #instantiate a model as per normal
    model = DnCNN(channels=3, num_of_layers=17)
    if local_train_pparam.local_rank == 0:
        print(f'{model}')        
    ...
    L1_loss = torch.nn.L1Loss().to(local_train_pparam.device)

    #only assign model to local cuda device after fully shard
    model.to(local_train_pparam.device) 
    ...
    dSampler = DistributedSampler(
        ...
        rank=local_train_pparam.local_rank,
        shuffle=True, # Recommended for FSDP
        seed=42
    )
    
    #data loader's shuffle is done by the distributed sampler
    trainning_dataloader = DataLoader(dataset= dataset, batch_size=local_train_pparam.batch_size, shuffle=False, sampler=dSampler, num_workers=2, pin_memory=True, drop_last=False) #num_worker forced more paralleism between learning and data loading (gpu v cpu tasks). seems 1 is ok, more than 1 causes lot of memory usage (160GB)
    
    #doing epoch stuff
    for epoch in range(10):
        timenow = datetime.datetime.now()
        trainning_dataloader.sampler.set_epoch(epoch) #init sampler
        ...
        #loop data inside daloader to train
        for i, data in enumerate(tqdm_dataloader, 0):
            source = data[0].to(local_train_pparam.device) #to(self.gpu_id)
            targets = data[1].to(local_train_pparam.device) #to(self.gpu_id)

            if args.explicit_prefetching:
                model.unshard()
            #loop dataloader to foward pass
            output = model(source)
        ...
        print(f'epoch {epoch} rank {local_train_pparam.local_rank}, epoch time {deltatime.total_seconds()}')
...
if __name__ == "__main__":
    ...
    """ torchrun --nproc_per_node 2 ./pyFSDP2/fsdp2_basic.py --save-every=20 --batch-size=10 """
